...
"""
    ...
    def compile_cpp(self, file_path: str) -> Tuple[bool, str, str]:
        """
        ...
        if not compiler_available:
            return False, "", "C++ compiler not found. Please install g++, clang++, or Visual Studio Build Tools."
    ...
    def run_cpp(self, input_data: str = None) -> Tuple[str, str, int, bool]:
        """
    ...
    def parse_error_output(self, stderr: str, stdout: str = "") -> Optional[ExecutionError]:
        ...
        # Look for specific C++ runtime errors
        elif "terminate called" in error_text:
            error_type = "runtime_error"
            error_message = "Program terminated unexpectedly"
            
            # Look for exception information
            if "exception" in error_text.lower():
                exception_match = re.search(r'what\(\):\s*(.+)', error_text)
                if exception_match:
                    error_message = f"Exception: {exception_match.group(1).strip()}"
    ...
    def validate_cpp_code(self, code: str) -> Optional[str]:
        """
    ...
    def execute(self, code: str, input_data: str = None) -> ExecutionResult:
        ...
        try:
            ...
            validation_error = self.validate_cpp_code(code)
            if validation_error:
            ...
            file_path = self.create_temp_file(wrapped_code, "program.cpp")
            
            # Compile C++ code
            compile_success, compile_stdout, compile_stderr = self.compile_cpp(file_path)
            ...
            # Run compiled C++ executable
            stdout, stderr, return_code, timed_out = self.run_cpp(input_data)
